...
from app.api.v1.endpoints import ocr, interaction, recommendation, supplement, health # 각 기능별 엔드포인트 모듈 임포트

api_router = APIRouter() # 메인 API 라우터 생성

api_router.include_router(health.router, prefix="/health", tags=["health"]) # 헬스 체크 라우터 등록
api_router.include_router(ocr.router, prefix="/ocr", tags=["ocr"]) # OCR 관련 라우터 등록
...
api_router.include_router(supplement.router, prefix="/supplement", tags=["supplement"]) # 영양제 관련 라우터 등록
