...
export default function DrugDetailScreen({navigation}: Props) {
  ...
  const addRecommendationDrug = useDrugStore(
    (state) => state.addRecommendationDrug,
  );
          ...
          <View style={styles.tabContentContainer}>
            ...
            <InfoRow
              ...
              value={formatIngredients(
                data.drugInfo?.parsedIngredients,
                data.drugInfo?.rawIngredients,
              )}
            />
          ...
          <View style={styles.tabContentContainer}>
            <View style={styles.permitGrid}>
              <View style={styles.permitGridItemLeft}>
                <InfoRow
                  label="품목기준코드"
                  value={data.permitInfo?.itemSeq}
                />
              </View>
              <View style={styles.permitGridItemRight}>
                <InfoRow
                  label="유효기간"
                  value={data.permitInfo?.validityPeriod}
                />
              </View>
              <View style={styles.permitGridItemLeft}>
                <InfoRow
                  label="저장방법"
                  value={data.permitInfo?.storageMethod}
                />
              </View>
              <View style={styles.permitGridItemRight}>
                <InfoRow
                  label="포장단위"
                  value={data.permitInfo?.packageUnit}
                />
              </View>
            </View>
          ...
          <View style={styles.tabContentContainer}>
            <View style={styles.permitSectionHeader}>
              <View
                style={[styles.verticalBar, {backgroundColor: "#111827"}]}
              />
              <Text style={styles.permitSectionTitle}>
                DUR(의약품 안심 서비스)
              </Text>
            </View>
              ...
              <>
                  ...
                  <View style={styles.durSection}>
                    ...
                    {contraindications.map((item, index) => (
                      <View key={index} style={styles.durItemContainer}>
                        <InfoRow
                          label="원료명1"
                          value={item.ingrName1}
                          noBorder
                        />
                        <InfoRow
                          label="원료명2"
                          value={item.ingrName2}
                          noBorder
                        />
                        <InfoRow
                          label="금기내용"
                          value={item.contraindReason}
                          noBorder
                        />
                      </View>
                    ))}
                ...
                {durWarnings.length > 0 &&
                  (() => {
                    ...
                    const byType = warnings.reduce<
                      Record<string, typeof warnings>
                    >((acc, w) => {
