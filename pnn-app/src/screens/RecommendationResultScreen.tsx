...
const DEFAULT_ANALYSIS_FALLBACK =
  '입력하신 처방약과 기저 질환을 분석하여 가장 안전하고 적합한 영양 성분을 선별했습니다.';
...
export default function RecommendationResultScreen() {
  ...
  const result: RecommendationResponse | undefined = route.params?.result;
  const bottomInset = useEffectiveBottomInset();

  const nutrients = result?.recommendedNutrients ?? [];
  const hasNutrients = nutrients.length > 0;

  const renderNutrientCard = (nutrient: RecommendedNutrient, index: number) => (
    <View key={`${nutrient.nameEn}-${index}`} style={styles.card}>
      ...
      >
        ...
        <View style={styles.analysisCard}>
          ...
          <Text style={styles.analysisText}>
            {result?.interactionAnalysis || DEFAULT_ANALYSIS_FALLBACK}
          </Text>
        </View>

        {hasNutrients ? (
          nutrients.map((item, index) => renderNutrientCard(item, index))
        ) : (
          <View style={styles.emptyCard}>
            <Ionicons name="information-circle-outline" size={48} color="#9CA3AF" />
            <Text style={styles.emptyTitle}>추천 가능한 성분이 없습니다.</Text>
            <Text style={styles.emptyText}>
              입력하신 약물과의 안전성을 최우선으로 고려한 결과, 현재 확실하게 추천해 드릴 수 있는
              성분이 부족합니다.
            </Text>
          </View>
        )}
        ...
        <View style={styles.disclaimerContainer}>
          <Text style={styles.disclaimerText}>
            ※ 본 추천은 AI에 의한 참고용 정보이며, 의학적 진단이나 치료를 대신할 수 없습니다. 새로운
            영양제를 복용하기 전에는 반드시 담당 의사 또는 약사와 상담하시기 바랍니다.
          </Text>
        </View>
