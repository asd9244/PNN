...
export default function RecommendationScreen({ navigation }: Props) {
  const recommendationDrugs = useDrugStore((state) => state.recommendationDrugs);
  const removeRecommendationDrug = useDrugStore((state) => state.removeRecommendationDrug);
  ...
  const handleRecommend = async () => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : '영양제 추천 중 오류가 발생했습니다.';
    ...
    <View style={styles.safeArea}>
      ...
      >
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.sectionSubtitle}>현재 복용 중인 약을 모두 추가해주세요.</Text>
          <View style={styles.buttonRow}>
            <ActionCard
              title="상세검색"
              iconName="search-outline"
              onPress={() =>
                navigation.navigate('DrugSearch', { sourceScreen: 'Recommendation' })
              }
              compact
              iconColor="#2563EB"
              iconBgColor="#DBEAFE"
            ...
            <ActionCard
              ...
              onPress={() =>
                navigation.navigate('PillIdentify', { sourceScreen: 'Recommendation' })
              }
              compact
              iconColor="#7C3AED"
              iconBgColor="#EDE9FE"
        ...
        <View style={styles.section}>
          ...
          <TextInput
            style={styles.conditionTextInput}
        ...
        <View style={styles.summaryCard}>
          <View style={styles.summaryCardHeader}>
            ...
            <Text style={styles.summaryCardTitle}>처방약 요약 ({drugCount})</Text>
          </View>

          {drugCount === 0 ? (
            <Text style={styles.summaryCardPlaceholder}>처방약을 추가해주세요.</Text>
          ) : (
        ...
        <TouchableOpacity
          style={[styles.bottomButton, canRecommend && { backgroundColor: '#059669' }]}
        ...
        >
          {isRecommending ? (
            ...
            <Text style={[styles.bottomButtonText, canRecommend && { color: '#FFFFFF' }]}>
              맞춤 영양제 추천받기
            </Text>
          )}
        </TouchableOpacity>
